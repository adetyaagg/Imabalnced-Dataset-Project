...
# Use Under-sampler library to under-sampler sample the majority class
...
plt.ylabel("Values")
plt.xlabel("Class")
plt.title("Class Distribution of the Dataset - Under Sample")
plt.show()
...
# Gaussian Model
model = GaussianNB().fit(X_under, Y_under)
...
# Model Decision Tree
model = DecisionTreeClassifier(random_state=0).fit(X_under, Y_under)
...
for i in range(1, 11):
    for j in range(1, 6):
        model = RandomForestClassifier(n_estimators=i, max_depth=j, criterion="entropy", random_state=0).fit(X_under,
                                                                                                             Y_under)
...
model = RandomForestClassifier(n_estimators=optimal_trees, max_depth=optimal_depth, criterion="entropy",
                               random_state=0).fit(X_under, Y_under)
...
history = model.fit(X_under, Y_under, validation_split=0.05, epochs=50,
