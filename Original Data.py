...
for i in range(1, 11):
    for j in range(1, 6):
        model = RandomForestClassifier(n_estimators=i, max_depth=j, criterion="entropy", random_state=0).fit(x_train,
                                                                                                             y_train)
...
history = model.fit(x_train, y_train, validation_split=0.05, epochs=50,
